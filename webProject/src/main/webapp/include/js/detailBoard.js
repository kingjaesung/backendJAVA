...
let buttonCheck = ""; // 수정 버튼과 삭제 버튼을 구별하기 의한 변수
	...
	if (!dataCheck("#passwd", "#message", "비밀번호를")) return;
		...
		$.ajax({
			...
			if (boardNumber === 0) {
				...
				// 입력 필드에 모든 텍스트가 드래그로 강조 (텍스트 전체가 파란색 강조)
				...
				if (buttonCheck === "updateButton") {
					...
					console.log("삭제버튼 실행 ")
					if (confirm("정말 삭제하시겠습니까?")) {
						actionProcess("#dataForm", "get", "/board/deleteBoard.do")
					}
				}
			}
